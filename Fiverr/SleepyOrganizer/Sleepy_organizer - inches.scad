...
height_lower_linear = lower_height-radius_top;

parts(part);

assert(height>lower_height,"lower_height cannot be higher than height");
...
module parts(part) {
    ...
    else if (part == "divider_slope")
        divider_slope(length=divider_length);
...
module rotate_extrude2(angle=360, size=39.37) {
    module angle_cut(angle,size=39.37) {
        x = size*cos(angle/2);
        y = size*sin(angle/2);
        translate([0,0,-size])
...
module profile(length=5.906, border=false) {
    rotate([90,0,0])
...
module fitting(male=true, border=false, lower=false) {
    // shrink male piece a little bit
    gap = male ? gap : 0;
    gap_top = male ? gap_top : 0;
    connector_length = radius_bottom;
    height =lower ? height_lower_linear : height_linear;
    ...
    radius_top_gap = radius_top+(radius_bottom-radius_top)*gap_top/height;
    ...
    snap_height = height * snap_height_factor;
    ...
    multmatrix(m=[
        ...
        [0,0,0,1]]) {
        union() {
            linear_extrude(height=height-gap_top, scale=radius_top_gap/radius_bottom) {
                polygon([
...
module divider(length=3.937, border=false) {
    difference() {
...
module divider_lowered(length=3.937, lower=lower_height, radius1_factor=lowered_radius1_factor,
    ...
    height_lower = height_lower_linear;
...
module divider_slope(length=3.937, lower=lower_height, radius1_factor=lowered_radius1_factor) {
    radius1 = length*radius1_factor;
    ...
    length_ortho = length - 2*(length_round1);
    assert(length_ortho >= 0, "divider_lowered: radius1 too big or length too small");
    ...
    difference() {
        ...
        rotate([0,0,180])
            fitting(male=false);
        translate([0,-length])
            fitting(male=false,lower=true);
    }
}


module divider_bend(length=3.937, distance=bend_distance, radius_factor=bend_radius_factor) {
