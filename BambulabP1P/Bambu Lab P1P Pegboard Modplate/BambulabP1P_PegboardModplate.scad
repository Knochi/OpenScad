...
showRebuild=true; //show the rebuild
showGhosts=false;
isolate="none"; //["none","a1","a2","a3","a4","a5","a6","b3"]
...
module Bsite(){
  translate([-spcng-plateDims.x/2, spcng/2,0]) b1();
  translate([-spcng-plateDims.x/2,-spcng/2,0]) b2();
  b3();
  translate([-plateDims.x/2      ,-spcng/2,0]) b4();
  translate([plateDims.x/2+spcng , spcng/2,0]) b5();
  translate([plateDims.x/2       ,-spcng/2,0]) b6(); 
}
...
module a1(place=false,getFootprint=false){
  ...
  translate(cntrOffset){    
    if (getFootprint) translate([-ovDims.x,0])#square([ovDims.x,ovDims.y]);
      ...
      if (showGhosts) %rotate([0,+0.00,0]) translate([-125.71,-15.22,-0.01]) 
        import("pegboard a1.stl");  
      //ribs
...
module a3(place=false,getFootprint=false){
  ...
  if (getFootprint)
    ...
    translate([13.4,13.3,plateDims.z]) linear_extrude(1.4) text("A3",size=6.4,font="Arial:style=Bold");
     ...
     color("grey") translate([248.45,-332.52,0]) rotate(90) import("pegboard a3.stl");
  }
}
...
module a4(place=false, getFootprint=false){
  ...
  if (getFootprint)
    ...
    translate([19.1,-217.1,1.6]) linear_extrude(1.4) text("A4",size=6.4,font="Arial:style=Bold");
     ...
     color("grey") translate([-11.29,640.5,0]) rotate(-90) import("pegboard a4.stl");
  }
...
*b3(); //position, rotation(/)
module b3(){
    
  domePos=[[118.6,76.35,wallThck],[130.55,194.6,wallThck]]; //center pos
  domeDims=[[14.4,9.70,6.9],[9.21,14.4,6.9]];
  domeFlanks=[[4.4,4.4,+0,2.4],[0,0,4.4,4.4]];
  domeOffset=[[0,+0.35],[0,0.64]];
  
  edgRibDims=[plateDims.y-5,4.5,plateDims.z-wallThck]; //length,width,height
  topRibDims=[plateDims.y-182.1,5,plateDims.z-wallThck]; //rib to the site
  topRibXPos=[32.5,63.3,96.53,144.4,178.91,208.7];
  ...
  if (showRebuild) 
    ...
    {
    if (showGhosts) %rotate(-0.12) translate([-60.20,-330.68,0]) rotate(90) import("pegboard b3.stl");
    //plate
    linear_extrude(wallThck) difference(){
      square([plateDims.x,plateDims.y]);
      //grooves bottom
      for(i=[0:6])
        translate([i*tgDist.x+16.17,0]) tongGroove();
      //grooves left&right
      for(ix=[0,1],iy=[0:3]){
        rot= ix ? 90 : -90;
        translate([ix*plateDims.x,iy*tgDist.y+17]) rotate(rot) tongGroove();
    ...
    //top tap
    translate([115.4,plateDims.y-5,9.8]){
      cube([7.9,3,7.4]);
      translate([2,0,1.99]) rotate([90,0,0]) sqFrustum(size=[3.91,3.92,2.41],flanks=[0.28,0.28,0.28,0.28]);
    }
    
    //rib to the right
    translate([plateDims.x-edgRibDims.y,0,wallThck]) mirror([1,0,0]) rotate(90) rib(size=edgRibDims);
    //ribs to the top
    for (xPos=topRibXPos)
      translate([xPos,182.06,wallThck]) rotate(90) rib(size=topRibDims,chamfer=[1,1],tip=0);
    //screw Domes
    for (i=[0:len(domePos)-1])
      translate(domePos[i]) screwDome(domeDims[i],domeFlanks[i],domeOffset[i]);
    //rib in lower section
    translate([plateDims.x,84.60,wallThck]) 
      rotate(180) rib(size=[230.57,8,plateDims.z-wallThck],chamfer=[0,0], tip=68);
  }
  else{
     color("grey") translate([248.45,-332.52,0]) rotate(90) import("pegboard a3.stl");
  }
...
*b4();
module b4(){
  translate([298.89,639.58,0]) rotate(-90) import("pegboard b4.stl");
}

*b5();
module b5(){
  rotate(89.763)
    translate([-942.82-0.4,104.80,0]) import("pegboard b5.stl");
}
