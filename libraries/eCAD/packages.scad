...
//https://datasheet.lcsc.com/lcsc/1810231522_Lite-On-LTST-C230KGKT_C125106.pdf
  ovDims=[3.2,1.6,1.1];
  boardThck=0.28;
  diePos=[ovDims.x/2-1.35,0,0];
  dieDim=0.25;
  moldDim=[1.5,ovDims.y,ovDims.z-boardThck];
  moldFlankAng=5;
  copperThck=0.03;
  
  // P.C board
  color(whiteBodyCol) translate([0,0,copperThck]) linear_extrude(boardThck-copperThck*2) difference(){
    square([ovDims.x,ovDims.y],true);
    for (ix=[-1,1])
      translate([ix*ovDims.x/2,0]) circle(d=0.5+copperThck*2);
    }
   
  // molding  
  color(whiteBodyCol) translate([0,0,0]) rotate([-90,0,0]) linear_extrude(ovDims.y,center=true) 
    polygon([[-moldDim.x/2,-copperThck],[-moldDim.x/2+tan(moldFlankAng)*moldDim.z,moldDim.z],
             [moldDim.x/2-tan(moldFlankAng)*moldDim.z,moldDim.z],[moldDim.x/2,-copperThck]]);
  //copper
  for (im=[0,1])
    //castellated hole
    color(metalGoldPinCol) mirror([im,0,0]) translate([ovDims.x/2,0,0]){
      translate([0,0,copperThck/2]) linear_extrude(boardThck-copperThck) difference(){
        rotate(90) circle(d=0.5+copperThck*2);
        rotate(90) circle(d=0.5);
        translate([0.25+copperThck,0]) square(0.5+copperThck*2,true);
      }
      linear_extrude(copperThck) difference(){
        translate([-(ovDims.x-moldDim.x)/4,0]) square([(ovDims.x-moldDim.x)/2,ovDims.y],true);
        translate([0,0]) circle(d=0.5+copperThck);
        for (iy=[-1,1])
          translate([0,iy*ovDims.y/2]) circle(d=0.25);
      }
      translate([0,0,boardThck-copperThck]) linear_extrude(copperThck) difference(){
        translate([-0.5/2,0]) square([0.5,1],true);
        translate([0,0]) circle(d=0.5+copperThck);
      }
      for (iz=[0,1]) 
        rotate(90) rotate_extrude(angle=180) translate([0.25+copperThck/2,copperThck/2+iz*(boardThck-copperThck)]) circle(d=copperThck);
    }
  //mask
  color(greenBodyCol) translate([0,0,boardThck-copperThck]) linear_extrude(copperThck/2){
    square([0.25,ovDims.y],true);
    translate([(0.25+0.6)/2,0]) square(0.6,true);
    }
}
